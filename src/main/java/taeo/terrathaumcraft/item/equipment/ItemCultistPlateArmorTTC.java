...
import taeo.terrathaumcraft.render.ModelKnightArmorTTC;
...
public class ItemCultistPlateArmorTTC extends ItemCultistPlateArmor {
	public ItemCultistPlateArmorTTC(ArmorMaterial enumarmormaterial, int j, int k) {
	...
	public ModelBiped getArmorModel(EntityLivingBase entityLiving, ItemStack itemStack, int armorSlot)
	{
		...
		if (this.model1 == null) {
			this.model1 = new thaumcraft.client.renderers.models.gear.ModelKnightArmor(1.0F);
		}
		if (this.model2 == null) {
			this.model2 = new thaumcraft.client.renderers.models.gear.ModelKnightArmor(0.5F);
		}
		if(ReferenceTTC.smartMovingInstalled)
		{
			try {
				Constructor smartModel = Class.forName("taeo.terrathaumcraft.render.ModelKnightArmorTTC").getConstructor(float.class);
				if (this.model1Smart == null) {
					this.model1Smart = (ModelBiped) smartModel.newInstance(1.0f); //new ModelKnightArmorTTC(1.0F);
				}
				if (this.model2Smart == null) {
					this.model2Smart = (ModelBiped) smartModel.newInstance(0.5f); // new ModelKnightArmorTTC(0.5F);
				}
