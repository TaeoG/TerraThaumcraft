...
public class WorldGenMinableTTC extends WorldGenerator
{
	//==========================================mp mod
	...
	public WorldGenMinableTTC(Block block, int j, Block layerBlock, int layerMeta, int rarity, int veinSize,
	...
	public boolean generate(World world, Random random, int x, int z, int min, int max)//obsorb default system
	{
		...
		if(mPChunkX != mPPrevX || mPChunkZ != mPPrevZ || mPPrevBlock != mPBlock || minableBlockMeta != mPPrevMeta)
		{
			if (!generateBeforeCheck())
			{
				...
				if (rarity == 1 || (rarity > 0 && rand.nextInt(rarity) == 0) || rarity <= numfails * ReferenceTTC.oreFailureLimitMulti)
				{
				...
				{
					if(ReferenceTTC.oreFailureLimit)
					numfails ++;
				}
			}
		}
